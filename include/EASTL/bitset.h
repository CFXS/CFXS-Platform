...
///
    ...
    struct BitsetBase {
        ...
        enum {
            ...
            kBitsPerWordShift =
                ((kBitsPerWord == 8) ? 3 : ((kBitsPerWord == 16) ? 4 : ((kBitsPerWord == 32) ? 5 : (((kBitsPerWord == 64) ? 6 : 7)))))
        };
...
// clang-format on
	...
	*/
    ...
    inline bitset<N, WordType>::reference::reference(const bitset& x, size_type i) :
        ...
        mnBitIndex(i & kBitsPerWordMask) { // We have an issue here because the above is casting away the const-ness of the source bitset.
                                           // Empty
    }
    ...
    inline bitset<N, WordType>::bitset(uint32_t value) : base_type(value) {
        if ((N & kBitsPerWordMask) ||
            ...
            mWord[kWordCount - 1] &=
                ~(static_cast<word_type>(~static_cast<word_type>(0)) << (N & kBitsPerWordMask)); // This clears any high unused bits.
    }
